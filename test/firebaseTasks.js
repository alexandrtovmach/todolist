const dbRef = require('../backend/db/firebaseDbConnect').test(),
    chai = require('chai'),
...
chai.use(chaiHttp);
describe('Tasks in FirebaseDB', () => {

    describe('change database', () => {
        it('it should change db to firebase', (done) => {
            chai.request(server)
            .get('/api/changedefaultdb/fb')
            .end((err, res) => {
    ...
    describe('/PUT task', () => {
        ...
        before((done) => {
            dbRef.once('value')
            .then((data) => {
                lastTask = Object.values(data.val()).pop();
                done();
            }) 
        });
        describe('without id', () => {
